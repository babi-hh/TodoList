...
Page({
  ...
  touchE: function (e) {
    ...
    if(e.changedTouches.length === 1){
      ...
      var disXt = Math.abs(disX)
      var txtStyle = disXt > delBtnWidth / 2 ? 'left:' + (disX > 0 ? '-': '') + delBtnWidth + 'rpx' : 'left: 0'
      // var txtStyle = disX > delBtnWidth/2 ? 'left:-' + delBtnWidth + 'rpx' : ''
      // 获取手指触摸的是哪一项
      var index = e.currentTarget.dataset.index
      var list = _this.data.curLists
      list[index].txtStyle = txtStyle
  ...
  onLoad: function () {
    ...
    /*// Ajax 请求
    wx.request({
        url: 'https://dev.seldoon.pw/index.php',
        success: function(res) {
            console.log(res)
        }
    }),
  ...
  showToast: function(msg, icon) {
    wx.showToast({
      title: msg,
      icon: typeof icon === 'undefined' ? 'success' : 'none',
      duration: 1500
    });
  }
})
